...
import '../../../widget/dialog_loading.dart';
...
class _EditRoomState extends State<EditRoom> {
  ...
  onSetInitRoom() async {
    context
        .read<CategoryBloc>()
        .add(SelectCategoryEvent(category: widget.category));
    roomNameController.text = widget.room.roomName;
    priceController.text = widget.room.price.toString();
    descriptionController.text = widget.room.descriptionRoom;
    totalGuestController.text = widget.room.totalGuest.toString();
    totalRoomController.text = widget.room.totalRoom.toString();
    roomSizeController.text = widget.room.roomSize.toString();
    List<File> files = [];
    for (var i = 0; i < widget.room.listImageDetail.length; i++) {
      String path = widget.room.listImageDetail[i];
      File file = await TransfromImage.convertUrlToFile(path);
      files.add(file);
    }
    // ignore: use_build_context_synchronously
    context.read<ManageRoomBloc>().add(SetImageDetailRoomEvent(files: files));
  }
  ...
  onDeleteRoom(BuildContext context) {
    context.read<ManageRoomBloc>().add(
          DeleteRoomEvent(token: widget.token, roomModel: widget.room),
        );
  }
                          ...
                          children: [
                            InkWell(
                              ...
                              child: Container(
                                ...
                                child: state.imageCoverRoom.path.isNotEmpty
                                    ...
                                    : ShowImageNetwork(
                                        pathImage: widget.room.imageCover,
                                      ),
                              ),
                            ),
                            Container(
                            ...
                            ElevatedButton(
                              onPressed: () => dialogConfirm(
                                context,
                                onDeleteRoom,
                                'คุณแน่ใจที่จะลบห้องพักรายการนี้ใช่หรือไม่',
                              ),
                              style: ElevatedButton.styleFrom(
                                primary: AppConstant.bgCancelActivity,
                              ),
                              child: const Text('ลบข้อมูลห้องพัก'),
                            ),
                            ElevatedButton(
                              onPressed: () {
                                if (_formKey.currentState!.validate() &&
                                  ...
                                  RoomModel roomModel = RoomModel(
                                    ...
                                    imageCover: widget.room.imageCover,
                                    listImageDetail:
                                        widget.room.listImageDetail,
                                    businessId: categoryState
